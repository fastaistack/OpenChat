...
# 创建会话并返回信息
def create_session(user_id: str,model_selected:str):
    try:
        with SessionLocal() as db:
            session_id = str(uuid.uuid4()).replace("-", "")
            # time_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
            datetime_info = datetime.datetime.now()
            session = models.ChatSession(id=session_id, user_id=user_id, session_name="新建会话", create_time=datetime_info, update_time=datetime_info,model_selected = model_selected)
            db.add(session)
            db.commit()
            db.refresh(session)
            return session
...
# 更新会话选择的模型
def update_session_model_selected(session_id: str, user_id: str, model_selected: str):
    try:
        with SessionLocal() as db:
            session_info = db.query(models.ChatSession).filter(and_(models.ChatSession.id == session_id, models.ChatSession.user_id==user_id)).first()
            session_info.model_selected = model_selected
            session_info.update_time = datetime.datetime.now()
            db.commit()
        return True
    except Exception as ex:
