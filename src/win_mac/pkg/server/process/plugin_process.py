...
def create_session_plugin(session_plugins: List[schemas.SessionPluginBase]):
    ...
    for session_plugin in session_plugins:
        ...
        if db_session_plugin:
            ...
            if not session_plugin.plugin_param:
...
def plugins_init(user_id: str):
    plugins = [
        ...
        {"plugin_logo": "extension",
         ...
         {"plugin_logo": "extension",
         "plugin_key": "openrouter",
         "plugin_name_en": "openrouter",
         "plugin_name_cn": "openrouter",
         "plugin_path": "pkg.plugins.chat_model_plugin.default_openai",
         "plugin_order": 0,
         "plugin_type": "model",
         "plugin_status": False,
         "description_en": "openrouter model",
         "description_cn": "openrouter模型"},
         {"plugin_logo": "extension",
         "plugin_key": "volcengine",
         "plugin_name_en": "volcengine",
         "plugin_name_cn": "volcengine",
         "plugin_path": "pkg.plugins.chat_model_plugin.default_openai",
         "plugin_order": 0,
         "plugin_type": "model",
         "plugin_status": False,
         "description_en": "volcengine model",
         "description_cn": "volcengine模型"},
         {"plugin_logo": "extension",
         "plugin_key": "tencent",
    ...
    try:
        with SessionLocal() as db:
            ...
            if not exec_result:
                ...
                if 'style_search' in setting: # searxng单独处理
                    if setting['style_search'] == 'searxng':
                        searxng_list = [
                        ...
                        import random
                        for _ in range(3):
                            random_index = random.randint(0, len(searxng_list) - 1)
                            searxng_url = searxng_list[random_index]
                            setting['searxng_url'] = searxng_url
                            exec_result = load_model_result.call(item, setting, content_setting)
                            if exec_result:
                                return exec_result
            return exec_result
