...
def pdf_to_base64_json_file(pdf_path,image_save_path, page_num = None):
    import json
    import fitz
    import os
    from pkg.plugins.translator.utils import image_to_base64
    zoom = 4.0 # 图片缩放系数
    doc = fitz.open(pdf_path)

    base64_dict = {}
    if page_num: # page_num不为空的时候，传值为第几页
        page = doc[0]
        rotate = int(0)
        trans = fitz.Matrix(zoom, zoom).prerotate(rotate)
        pix = page.get_pixmap(matrix=trans, alpha=False)
        # img_save_path = os.path.join(image_save_path,'%s.png' % pg )  
        img_save_path = os.path.join(image_save_path,'0.png' )
        pix.save(img_save_path)
        base64_string = image_to_base64(img_save_path)
        base64_dict[page_num] = base64_string
        ...
        for pg in range(doc.page_count):
            ...
            pix.save(img_save_path)
            base64_string = image_to_base64(img_save_path)
            base64_dict[f'{pg}'] = base64_string
        
    base64_list_path = os.path.join(image_save_path,'base64.json')    
    if os.path.exists(base64_list_path):
        try:
            with open(base64_list_path, 'r', encoding='utf-8') as f:
        ...
        existing_data = {}
    existing_data.update(base64_dict)
    with open(base64_list_path, 'w', encoding='utf-8') as f:
        json.dump(existing_data, f, ensure_ascii=False, indent=2)
    return base64_dict
        
def pdf_to_base64(pdf_path,image_save_path):
    import fitz
    import os
    from pkg.plugins.translator.utils import image_to_base64
    zoom = 8.0 # 图片缩放系数
