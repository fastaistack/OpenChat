...
import json
from pkg.server.process.process_setting import set_setting_config,get_setting_config
...
async def update_websearch_plugin_param(session_id: str, item: schemas.UserPluginWebSearchParamUpdateInfo, headers=Depends(get_headers)):
    ...
    try:
        ...
        if update_result:
            # # 更新全局配置
            # setting_list = json.loads(get_setting_config('web_search'))
            # for i,setting in enumerate(setting_list):
            #     if setting['style_search'] == item.style_search:
            #         setting_list[i]["web_api_key"] = item.web_api_key
            #         setting_list[i]["searxng_url"] = item.searxng_url
            #         setting_list[i]["embedding_model_id"] = item.embedding_model_id
            #         setting_list[i]["retrieve_topk"] = item.retrieve_topk
            #         setting_list[i]["template"] = item.template
            # set_setting_config('web_search',json.dumps(setting_list))
...
async def update_sensitive_plugin_param(item: schemas.SensitiveSettingUpdateInfo, session_id: str, headers=Depends(get_headers)):
    ...
    try:
        ...
        if update_result:
            # # 更新全局配置
            # config_key = {"interval_tokens": item.baidu_api.interval_tokens, "api_key": item.baidu_api.api_key, "secret_key": item.baidu_api.secret_key}
            # set_setting_config('sensitive',json.dumps(config_key))
