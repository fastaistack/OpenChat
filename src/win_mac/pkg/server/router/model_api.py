...
import json
from pkg.server.process.process_setting import set_setting_config
...
# 已下载（本地）模型列表
@router.get("/download/list_clear", response_model=ModelBaseInfoResponse)
async def download_list_clear():
    result = ModelBaseInfoResponse
    model_list = process_model.get_download_model_list()
    if len(model_list) <= 0:
        return result.success([])
...
async def add_model_to_list(item: NewModelItem):
    try:
        model_info = process_model.insert_model_list(item.model_key, item.name, item.model_type)
        return ModelUrlInfoResponseInfo.success({
            "id": model_info.id,
        ...
        return ModelUrlInfoResponseInfo.error(f"添加失败: {str(e)}")

@router.delete("/deleteModel")
async def delete_model_from_list(id: int):
    try:
        success = process_model.delete_model_by_id(id)
        if success:
            return ModelUrlInfoResponseInfo.success(None)
        else:
...
async def load_model(info: ModelIdInfo):
    ...
    #         return result.fail(status.OPENCHAT_MODEL_LOAD_FAILED_ERROR.code, status.OPENCHAT_MODEL_LOAD_FAILED_ERROR.errmsg)
    load_flag, error = process_model.load_model(info.id, info.precision_selected, info.type)
    # 更新全局配置
    config_key = {"id":info.id, "model":info.precision_selected}
    set_setting_config('model_selected',json.dumps(config_key))
    if not load_flag:
        # return result.fail(status.OPENCHAT_MODEL_LOAD_FAILED_ERROR.code, status.OPENCHAT_MODEL_LOAD_FAILED_ERROR.errmsg)
        return result.fail(error.code, error.errmsg)
