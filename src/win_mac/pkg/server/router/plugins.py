...
async def getsettings(model: ModelGetRequest, db: Session = Depends(dbcrud.get_db), headers=Depends(get_headers)):
    try:
        ...
        # 获取全局模型信息
        # global_plugin_param = gvar.get_model_setting()
        from pkg.server.process import process_setting
        global_plugin_param = process_setting.get_setting_config('model_param')
        ...
        if db_query_session_plugin:
            ...
            # if global_plugin_param: # 必须要写入session对应的plugin中，不然chat时无法获取该参数
            #     db_query_session_plugin.plugin_param = global_plugin_param
            #     session_update = {"session_id": session_id, "plugin_id": db_query_session_plugin.plugin_id,
            #                         "update_val": {"plugin_param": global_plugin_param, "session_status": True}}
            #     db_update_session_plugin = dbcrud.update_session_plugin([SessionUpdateReq(**session_update)])
            # gvar.set_model_setting(db_query_session_plugin.plugin_param)
            ...
            if plugin_param_list:
                ...
                if global_plugin_param:
                    plugin_param = global_plugin_param
                plugin_id = db_query_plugin.plugin_id
...
async def setsettings(model: ModelPluginSetRequest, db: Session = Depends(dbcrud.get_db), headers=Depends(get_headers)):
    try:
        ...
        # 设置全局模型超参数
        # gvar.set_model_setting(plugin_param)
        # from pkg.server.process import process_setting
        # process_setting.set_setting_config('model_param',plugin_param)
