...
from ..process import process_chat, process_model,process_setting
import time
...
class ChatSessionUpdateModelRequestInfo(BaseModel):
    id: str
    model_selected: str
    ...
    try:
        ...
        for session_item in session_list:
            item = schemas.ChatSessionInfo(id=session_item.id, session_name=session_item.session_name,
                ...
                model_selected=session_item.model_selected)
...
async def get_new_session_info(headers=Depends(get_headers)):
    ...
    try:
        # 获取全局配置信息中的模型
        model_selected = process_setting.get_setting_config('model_selected')
        create_result = process_chat.create_session(headers[const.HTTP_HEADER_USER_ID],model_selected)
...
@router.put("/session/update/model_selected")
async def update_session_name(item: ChatSessionUpdateModelRequestInfo, headers=Depends(get_headers)):
    result = server_schema.CommonResponse
    try:
        session_info = process_chat.get_session_info(item.id, headers[const.HTTP_HEADER_USER_ID])
        if session_info is None:
            return result.fail(StatusCodeEnum.DB_NOTFOUND_ERR.code, StatusCodeEnum.DB_NOTFOUND_ERR.errmsg)
        return result.success({"result": process_chat.update_session_model_selected(item.id, headers[const.HTTP_HEADER_USER_ID], item.model_selected)})
    except Exception as ex:
        log.error(f"update_session_name error, {str(ex)}")
        return result.fail(StatusCodeEnum.UNKNOWN.code, StatusCodeEnum.UNKNOWN.errmsg)
    
@router.get("/session/model_selected/{session_id}")
async def get_session_model_selected(session_id:str, headers=Depends(get_headers)):
    result = ChatSessionListResponse
    try:
        if session_id == 'default':
            model_selected = process_setting.get_setting_config('model_selected')
            return result.success(model_selected)
        else:
            session_info = process_chat.get_session_info(session_id, headers[const.HTTP_HEADER_USER_ID])
            return result.success(session_info.model_selected)
    except Exception as ex:
        log.error(f"chat.get_session_list error,{str(ex)}")
        return result.fail(StatusCodeEnum.UNKNOWN.code, StatusCodeEnum.UNKNOWN.errmsg)
