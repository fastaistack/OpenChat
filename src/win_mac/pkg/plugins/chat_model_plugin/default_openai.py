    ...
    for chunk in stream:
        ...
        if getattr(delta, 'reasoning_content', None):
            ...
            if "<think>" in answer_content:
                # 检查是否闭合
                think_start = answer_content.find("<think>")
                think_end = answer_content.find("</think>")
                if think_end != -1:
                    # 提取思考与回答内容
                    reasoning_content = answer_content[think_start + 7 : think_end]
                    response_content = answer_content[think_end + 8:]

                    content_setting["output_think"] = reasoning_content
                    content_setting["output_answer"] = response_content
                else:
                    # 还没闭合，继续收集思考内容
                    content_setting["output_think"] = answer_content[think_start + 7:]
                    content_setting["output_answer"] = ""
            else:
                # 不含 <think>，正常回答
