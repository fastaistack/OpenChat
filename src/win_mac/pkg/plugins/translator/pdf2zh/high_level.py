    ...
    **kwarg: Any,
    ...
    with tqdm.tqdm(total=total_pages) as progress:
        for pageno, page in enumerate(PDFPage.create_pages(doc)):
            ...
            # pix = doc_zh[page.pageno].get_pixmap(matrix=trans, alpha=False)
            # #-----------------------生成高分辨率图片----------------------------
            ...
            # # -------------图像绘制--------------
            # doclayout_model.save_layout_as_image(image,"./output/yoloLayout.png")
...
def translate_stream(
    ...
    **kwarg: Any,
    ...
    for xref in range(1, xreflen): # fix code=7: cannot find object in xref (5 0 R)
        try:
            _ = doc_zh.xref_object(xref)
        except:
    ...
    **kwarg: Any,
    ...
    for file in files:
        ...
        for full_file, temp_file in translate_stream(s_raw, **locals()):
            ...
            gvar.set_complated_page_count(i)
            i = i + 1
