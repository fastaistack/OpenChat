...
def checkout_translate_item():
    ...
    try:
        ...
        for item in translate_items:
            ...
            item.translated_file_path = ""
        
        # 提交更改
        db.commit()
    except Exception as e:
        log.error(f"更新翻译项状态时出错: {str(e)}")
        db.rollback()
...
def get_translation_messages(text: str, base_lang: str, target_language: str = 'English') -> list:
    ...
    if glossary == -1:
        ...
        # 减少提示词中的术语说明
        appendix_glossary = '''8. [Term Processing Protocol]

                                (1). Placeholder Recognition
                                - Format: __TERM_number_
                                - Examples: __TERM_1_, __TERM_2_, __TERM_3_

                                (2). Processing Rules
                                - Treat placeholders as atomic units
                                - Maintain placeholder integrity and format
                                - Do not explain, replace, or delete placeholders
                                - Preserve placeholder positions during translation

                                (3). Output Requirements
                                - Placeholders must remain unchanged
                                - Ensure appropriate spacing around placeholders
                                - Maintain original placeholder order

                                (4). Error Handling
                                - If placeholder format is incorrect, keep as is
                                - If placeholder is damaged, attempt to restore to standard format

                                [Examples]
                                Input: This is a __TERM_1_ example with __TERM_2_
                                Output: 这是一个 __TERM_1_ 示例，包含 __TERM_2_

                                [Important Notes]
                                - Placeholders are critical components of the terminology system
                                - Never attempt to interpret or modify placeholder content
                                - Always preserve the exact format: __TERM_number_
                                - Ensure proper spacing before and after placeholders
                                - Maintain placeholder integrity throughout all processing steps'''
...
def convert_docx_first_page_to_image(docx_path, output_image_path=None):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            log.info(f"转换完成！图片已保存至: {output_image_path}")
            return output_image_path
            
    except Exception as e:
        log.error(f"转换过程中出现错误: {str(e)}")
        raise
...
def pdf_first_page_to_image(pdf_path, output_image_path=None, dpi=200):
    ...
    try:
        ...
        # 设置poppler路径（如果需要）
        poppler_path = None
        if os.path.exists("./_internal/poppler-0.89.0/bin"):
            poppler_path = "./_internal/poppler-0.89.0/bin"
