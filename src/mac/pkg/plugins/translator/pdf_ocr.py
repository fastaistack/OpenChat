...
def pdf_to_image(pdf_path,file_id):
...
def calculate_fontsize(text, w, h):
    padding_ratio = 0.05
    effective_w = w * (1 - 2 * padding_ratio)
    effective_h = h * (1 - 2 * padding_ratio)

    if effective_w <= 0 or effective_h <= 0 or len(text.strip()) == 0:
        return 16

    avg_char_width_ratio = 0.5
    avg_line_height_ratio = 1.2

    max_chars_per_line = effective_w / avg_char_width_ratio

    if len(text) <= max_chars_per_line:
        fontsize = effective_w / (len(text) * avg_char_width_ratio)
    else:
        lines_needed = len(text) / max_chars_per_line
        fontsize_by_height = effective_h / (lines_needed * avg_line_height_ratio)
        fontsize_by_width = effective_w / (max_chars_per_line * avg_char_width_ratio)
    ...
    for page_box in page_box_list:
        ...
        for txt in texts:
            if txt is not None:
                ...
                fontsize = calculate_fontsize(insert_text, int(x1-x0), int(y1-y0))
