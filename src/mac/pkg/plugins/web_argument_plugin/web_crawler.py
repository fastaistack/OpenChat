...
class WebScraper:
    def __init__(self, user_agent='Windows'):
        # Initialize the scraper with a user agent (default is 'Windows')
    ...
    def get_webpage_html(self, url):
        ...
        try:
            ...
            # requests爬虫【中文乱码】的3种解决方法 https://blog.csdn.net/2301_82000445/article/details/135840967
    ...
    def extract_main_content(self, html_soup, rule=0):
        ...
        for tag in html_soup.find_all(tag_rule):
            ...
            # 去除正则重复匹配信息
            if len(main_content) > 0 and tag_text in main_content[-1]:
                continue

            # if tag_text and len(tag_text.split()) > 10:   #适用于英文提取
    ...
    def scrape_url_trafilatura(self, url):
        # Public method to scrape a URL and extract its main content
        webpage_html = self.get_webpage_html(url)
        html_content = webpage_html.text
        main_content = extract(html_content, include_tables=False, include_comments=False)
        reset_caches()
        return main_content
...
if __name__ == "__main__":
    ...
    test_url = "https://www.meet99.com/jingdian-taishan.html"
    main_content = scraper.scrape_url(test_url, 0)
