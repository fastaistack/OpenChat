...
def download_files_from_server(server_url, path_list, download_dir=DOWNLOAD_DIR):
    ...
    # 准备请求数据
    # url = f"{server_url}/download/files"
    # payload = {"user_id":db_query.user_id,"path_list": path_list} # win
    url = f"{server_url}/download/files_mac"
    payload = {"user_id":db_query.user_id,"path_list": [["mac", "OpenChat.dmg"]]} # mac
...
def check_status(task_id: str):
    ...
    if task and task.is_alive():# 正在下载
        ...
        result.errMsg = status.OK.errmsg
        result.errCode = status.OK.code
        print(gvar.get_update_progress())
        ...
        if progress >= 99:
            ...
            result.errMsg= status.ERROR.errmsg
            result.errCode = status.ERROR.code
            result.resData = json.dumps ({"task id": task_id, "status": True})
            download_task.clear()
            return result
