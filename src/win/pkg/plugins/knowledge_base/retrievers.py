...
def call(
    ...
    '''
    logger.debug(f"knowledge base query params: request: [{request.dict().items()}]; setting: [{setting.items()}]")
    # get question
    question = request.message
    knowledge_id = request.reference_info.get("knowledge_id", "")
    knowledge_name = request.reference_info.get("knowledge_name", "")
    logger.debug(f"request with knowledge_id: [{knowledge_id}]; knowledge_name: [{knowledge_name}]")
    ...
    try:
        ...
        knowledge_params_dict["query_param"]["file_id"] = ""
        ...
        logger.error(f"Error happened in knowledge base plugin with request [{request.dict().items()}], error: [{e}]")
